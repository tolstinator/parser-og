select
    ...
    pg.description
from
    games g left outer join games_properties gp on g.id = gp.id_game
...
select
    ...
    pf.description
from
    files_properties fp left outer join games_files gf on fp.id_file = gf.id
...
select
    ...
    gf.cause_unload
from
    games g left outer join games_files gf on g.id = gf.id_game
...
from
    games_screenshots gs left outer join games g on gs.id_game = g.id
where
        gs.cause_unload!=''
   or
        gs.cause_load!=''
...
select
        sum(gs.size)/1024/1024, count(1), max(gs.size)
from
    games_screenshots gs left outer join games g on gs.id_game = g.id
;

select
    gw.link_id as "link_id",
    gw.name as "name",
    ...
    games g left outer join genres gnr on g.id_genre = gnr.id
            ...
            left outer join (select
                             ...
                             from
                                 games_alt_names gan
                             group by
                                 gan.id_game) gan on g.id=gan.id_game
            left outer join (select
                                 gd.id_game,
                                 string_agg(c.name,', ') as "name"
                             from
                                 games_developers gd left outer join companies c on gd.id_company = c.id
                             group by
                                 gd.id_game) gd on g.id=gd.id_game
            left outer join (select
                                 gp.id_game,
                                 string_agg(c.name,', ') as "name"
                             from
                                 games_publishers gp left outer join companies c on gp.id_company = c.id
                             group by
                                 gp.id_game) gp on g.id=gp.id_game
            left outer join (select
                                 gf.id_game,
                                 sum(gf.size) as "size"
                             from
